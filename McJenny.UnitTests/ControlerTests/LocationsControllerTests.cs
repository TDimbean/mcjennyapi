...
namespace McJenny.UnitTests.ControlerTests
{
    ...
    public class LocationsControllerTests
    {
        ...
        public async Task GetLocation_HappyFlow_ShouldReturnLocation()
        {
            ...
            var loc = await _context.Locations.FindAsync(locId);
            var ttable = (await _context.Schedules.FindAsync(loc.ScheduleId)).TimeTable;
            ...
            var expected = new {
                ...
                loc.Street,
                loc.MenuId,
                OpenSince = loc.OpenSince.ToShortDateString()
            };
        ...
        // Cannot test, because GetEmployees implements .Include, which does not work in EFCore3Mock
        [TestMethod]
        public async Task GetEmployees_HappyFlow_ShouldReturnLocationsEmployees()
        {
            ...
            var positions = await _context.Positions
                .Select(p => new { p.PositionId, p.Title })
                .ToArrayAsync();

            var employees = await _context.Employees
                .Where(e => e.LocationId == location.LocationId)
                .ToArrayAsync();

            foreach (var emp in employees)
            {
                var pos = await _context.Positions.FindAsync(emp.PositionId);
                emp.Position = pos;
            }
        ...
        private static void Setup()
        {
            ...
            var managements = FakeRepo.Managements;
            ...
            {
                var menu = menus.SingleOrDefault(m => m.MenuId == loc.MenuId);
                var sch = schedules.SingleOrDefault(s => s.ScheduleId == loc.ScheduleId);
            ...
            foreach (var emp in employees)
            {
                var pos = positions.SingleOrDefault(p => p.PositionId == emp.PositionId);
                emp.Position = pos;
            }
            ...
            var managementsDbSetMock = dbContextMock.CreateDbSetMock(x => x.Managements, managements);
            ...
            dbContextMock.Setup(m => m.Managements).Returns(managementsDbSetMock.Object);
