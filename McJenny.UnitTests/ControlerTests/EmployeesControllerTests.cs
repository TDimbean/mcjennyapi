...
namespace McJenny.UnitTests.ControlerTests
{
    ...
    public class EmployeesControllerTests
    {
        ...
        public async Task GetEmployeesFiltered_FirstNameMatch_ShouldReturnFiltered()
        {
            ...
            foreach (var emp in employees)
            {
                var position = await _context.Positions.FindAsync(emp.PositionId);
        ...
        {
            // Arrange
            var posId = await _context.Employees
                .Select(e => e.PositionId)
                .FirstOrDefaultAsync();
            var filter = (await _context.Positions
                .FindAsync(posId))
                .Title;
        ...
        ////TODO Check when adding Management or Updating Employee that Location is not currently Managed
        //[TestMethod]
        //[Ignore]
        ...
        /// Helpers
        int GetFirstEmployeeId => _context.Employees.Select(d => d.EmployeeId).OrderBy(id => id).FirstOrDefault();
        int GenBadEmployeeId => _context.Employees.Select(d => d.EmployeeId).ToArray().OrderBy(id => id).LastOrDefault() + 1;
